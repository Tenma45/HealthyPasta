...
<v-card color="black" outlined  max-width="1200">
  <v-card-text class="white--text subtitle-1">
      <v-row>
          <v-col col="12" sm="3">
              <div class="text-center"><img :src="wait.pic" height="150"></div>
          </v-col>
                ...
                <v-card >
                    <v-card-text class="black--text pt-5 text-center">
                        ...
                        <v-card-actions>
                                ...
                                <v-btn @click="removerequest" color="black" class="font-weight-bold mx-auto" x-large outlined >
                                    ยืนยัน
                                </v-btn>
                        </v-card-actions>
                        </v-card-text>
...
const router = new VueRouter ({
    routes : [
        ...
        {path:'/course-pending', component:CourseWait},
...
new Vue({
    ...
    methods: {
      ...
      async fetchstatus(){
        try{
          const res = await axios.post('/fetchstatus',{
          })
           this.statusinfo=res.data
           if(res.data.coursestatus==1){
            this.fetchcourse()
           }
           else if(res.data.coursestatus==2){
             this.fetchwait()
           }
           else{
           router.push("/course2")
      ...
      async fetchwait(){
        try{
            const res = await axios.post('/fetchwait',{
          })
           this.wait=res.data
           router.push("/course-pending")
        }catch (err){
            console.log(err)
        }
      },
